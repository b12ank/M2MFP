...
train_data_path = r"E:\pycharmproject\raw_code\新工作\DCRV\train_merge"
test_data_path = rf"E:\pycharmproject\raw_code\新工作\DCRV\test_67"
result_path = r"E:\pycharmproject\raw_code\新工作\DCRV\result"
os.makedirs(result_path, exist_ok=True)
threshold = 0.7


def train_model(train_data_path):
    train_pos = pd.read_feather(f'{train_data_path}/positive_train_neg40.feather')
    train_neg = pd.read_feather(f'{train_data_path}/negative_train_neg40.feather')
    ...
    use_features = [i for i in use_features if i != "LogTime" and i != "ReportTime" and i != "HS_cnt"]
    ...
    LGB_MODEL_PARAMS = {"learning_rate": 0.02, "n_estimators": 500, "max_depth": 8,
                        ...
                        'importance_type': 'gain', "scale_pos_weight": 100}           # "is_unbalance": True, "scale_pos_weight": 10
    ...
    with open(os.path.join(model_save_path, "lightgbm.pkl"), "wb") as f:
        pickle.dump(model, f)
    ...
    importance_df.to_csv(os.path.join(model_save_path, 'feature_importance_gain.csv'), index=False)
...
def train_and_predict():
    ...
    # model_save_path = result_path
    # with open(os.path.join(model_save_path, "lightgbm.pkl"), 'rb') as f:
    #     model = pickle.load(f)
    ...
    submission.to_csv(os.path.join(result_path, f"DCRV_lightgbm_test67_submission_{threshold}.csv"), index=False)
