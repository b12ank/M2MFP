...
def process_single_sn(sn_file):
    processed_time_point_data_path = os.path.join(WORK_DIR, "bfse_processed_data")
    combined_sn_feature_data_path = os.path.join(WORK_DIR, "time_patch_processed")
    new_df = pd.read_feather(os.path.join(processed_time_point_data_path, sn_file))
    ...
    combined_df.to_feather(os.path.join(combined_sn_feature_data_path, sn_file))
...
if __name__ == "__main__":
    ...
    os.makedirs(combined_sn_feature_data_path, exist_ok=True)

    # Single process
    # sn_file_list = os.listdir(processed_time_point_data_path)[:4000]
    # exist_sn_file_list = os.listdir(combined_sn_feature_data_path)
    # file_name_list = [i for i in sn_file_list if i not in exist_sn_file_list and i.endswith("feather")]
    #
    ...
    sn_file_list = os.listdir(processed_time_point_data_path)[-20000:]
    exist_sn_file_list = os.listdir(combined_sn_feature_data_path)
