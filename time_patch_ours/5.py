...
def train_model(train_data_path):
    train_pos = pd.read_feather(f'{train_data_path}/positive_train.feather')
    train_neg = pd.read_feather(f'{train_data_path}/negative_train.feather')
    ...
    with open(os.path.join(model_save_path, "time_patch_model1-5.pkl"), "wb") as f:
        pickle.dump(model, f)
...
def predict(test_data_path, model):
    ...
    for test_file in tqdm(os.listdir(test_data_path)):
        ...
        test_df = test_df[model.feature_name_]         # 只有363维
...
def train_and_predict():
    ...
    with open(r"E:\pycharmproject\raw_code\work\time_patch_model.pkl", 'rb') as f:
        model = pickle.load(f)
    ...
    submission.to_csv(os.path.join(result_path, "model_test5-6_data_all_patch_submission_0.5.csv"), index=False)
...
if __name__ == "__main__":
    ...
    # # 测试集带标签
    # test = pd.read_feather(r"E:\pycharmproject\raw_code\work\time_patch_test\train_merge_test.feather")
    # with open(r"E:\pycharmproject\raw_code\work\time_patch_model.pkl", 'rb') as f:
    #     model = pickle.load(f)
    #
    # test["sn_name"] = test.index.get_level_values(0)
    # test["log_time"] = test.index.get_level_values(1)
    # y = test["label"]
    # X = test[model.feature_name_]
    # pred_result = model.predict(X)
    # pred_prob = model.predict_proba(X)
    #
    # precision = precision_score(y, pred_result)
    # recall = recall_score(y, pred_result)
    # f1 = f1_score(y, pred_result)
    #
