...
if __name__ == "__main__":
    # with open(r"E:\pycharmproject\raw_code\parity_features_all.pkl", "rb") as f:
    #     parity_dict = pickle.load(f)
    # # 修改三个路径
    # for i in range(1, 11):
    #     data_path = rf"E:\pycharmproject\data_all\folder{i}\type_A"
    ...
    #
    #     os.makedirs(processed_time_point_data_path, exist_ok=True)
    ...
    #         old_df = pd.read_feather(os.path.join(data_path, i))
    #         if i[:-8] in ticket_sn:
    #             old_df = old_df[old_df['LogTime'] <= ticket_sn_map[i[:-8]]]
    ...
    #             axis=1)
    #
    #         new_df.to_feather(os.path.join(processed_time_point_data_path, i))
    #         # 显式释放变量
    #         del old_df, new_df, manufacturer_df, retry_log_df, error_type_df, parity_features_df
    #
    #         # 强制垃圾回收
    #         gc.collect()
    processed_time_point_data_path = r"E:\pycharmproject\raw_code\work\bfse_processed_data"
    ticket_path = r"E:\pycharmproject\phase2_data\failure_ticket.csv"
    ...
    ticket_time = list(ticket['alarm_time'])
    ticket_sn_map = {sn: sn_t for sn, sn_t in zip(ticket_sn, ticket_time)}
    for idx, folder_name in enumerate(os.listdir(r"E:\pycharmproject\data_all")):
        with open(os.path.join(f"E:\pycharmproject\phase2_data\parity_dict_list", f"parity_features{idx}.pkl"), "rb") as f:
            parity_dict = pickle.load(f)
        data_path = rf"E:\pycharmproject\data_all\{folder_name}\type_A"
        for i in tqdm(os.listdir(data_path)):
            old_df = pd.read_feather(os.path.join(data_path, i))
            if i[:-8] in ticket_sn:
                old_df = old_df[old_df['LogTime'] <= ticket_sn_map[i[:-8]]]
