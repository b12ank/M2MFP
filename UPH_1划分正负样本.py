...
test_date_range = ("2024-06-01", "2024-08-01")

# processed_time_point_data_path = r"E:\pycharmproject\raw_code\work1\bfse_processed_data"
combined_sn_feature_data_path = r"E:\pycharmproject\raw_code\新工作\UPH全部样本\UPH全部特征构造"
ticket_path = r"E:\pycharmproject\raw_code\failure_ticket.csv"
train_data_path_UPH = r"E:\pycharmproject\raw_code\新工作\UPH全部样本\UPH_train"
test_data_path_UPH = rf"E:\pycharmproject\raw_code\新工作\UPH全部样本\UPH_test_67"         # 修改成test_date_range的范围
# train_data_path_time_point = r"E:\pycharmproject\raw_code\work1\time_point_train"
# test_data_path_time_point = r"E:\pycharmproject\raw_code\work1\time_point_test89"

os.makedirs(train_data_path_UPH, exist_ok=True)
os.makedirs(test_data_path_UPH, exist_ok=True)
...
def process_neg_file(args):
    ...
    if not pos_ticket_sn_map.get(sn_file[:-8]):
        ...
        if data_type == "UPH":
            ...
            if len(data) > 40:      # 负样本>80时取最后10个窗口拼随机70个窗口
...
def get_positive_train_data(data_type):
    ...
    for chunk_index, file_chunk in enumerate(split_test_files):
        ...
        if pos_data_all is not None:
            if data_type == "UPH":
                pos_data_all.to_feather(f'{train_data_path_UPH}/positive_train1-6_{chunk_index+1}.feather')
            elif data_type == "time_point":
...
def get_negative_train_data(data_type):
    ...
    for chunk_index, file_chunk in enumerate(split_test_files):
        ...
        if neg_data_all is not None:
            if data_type == "UPH":
                neg_data_all.to_feather(f'{train_data_path_UPH}/negative03-04_train1-5_{chunk_index+1}.feather')
            elif data_type == "time_point":
    ...
    for chunk_index, file_chunk in enumerate(split_test_files):
        ...
        for file in tqdm(file_chunk):
            if data_type == "UPH":
                ...
                data_tmp = data_tmp[data_tmp['ReportTime'] > datetime_to_timestamp(test_date_range[0])]
                data_tmp = data_tmp[data_tmp['ReportTime'] <= datetime_to_timestamp(test_date_range[1])]
                data_tmp = data_tmp.sort_values(by=['ReportTime'])[-40:]        # 每条dimm控制根据ReportTime排序的最后40行
            if data_tmp.empty:
                continue
        ...
        if test_data_all is not None:
            if data_type == "UPH":
                test_data_all.to_feather(os.path.join(test_data_path_UPH, f"test_data_{chunk_index+1}.feather"))
            elif data_type == "time_point":
